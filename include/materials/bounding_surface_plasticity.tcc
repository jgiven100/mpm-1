...
mpm::BoundSurfPlasticity<Tdim>::BoundSurfPlasticity(unsigned id, const Json& material_properties)
...
//! Initialise state variables
template <unsigned Tdim>
std::vector<std::string> mpm::BoundSurfPlasticity<Tdim>::state_variables() const {
...
template <unsigned Tdim>
Eigen::Matrix<double, 6, 6> mpm::BoundSurfPlasticity<Tdim>::compute_elastic_tensor(
...
template <unsigned Tdim>
Eigen::Matrix<double, 6, 6> mpm::BoundSurfPlasticity<Tdim>::compute_elasto_plastic_tensor(
...
template <unsigned Tdim>
void mpm::BoundSurfPlasticity<Tdim>::compute_stress_invariants(const Vector6d& stress,
                                                   double* p, double* q,
                                                   double* lode_angle,
                                                   double* M_theta) {
...
template <unsigned Tdim>
void mpm::BoundSurfPlasticity<Tdim>::compute_image_parameters(mpm::dense_map* state_vars) {
...
template <unsigned Tdim>
void mpm::BoundSurfPlasticity<Tdim>::compute_state_variables(
  ...
  if (yield_type == mpm::boundsurfplasticity::FailureState::Yield) {
...
//! Compute elastic tensor
template <unsigned Tdim>
void mpm::BoundSurfPlasticity<Tdim>::compute_p_bond(mpm::dense_map* state_vars) {
...
template <unsigned Tdim>
typename mpm::boundsurfplasticity::FailureState mpm::BoundSurfPlasticity<Tdim>::compute_yield_state(
  ...
  auto yield_type = mpm::boundsurfplasticity::FailureState::Elastic;
  ...
  if ((*yield_function) > tolerance_)
    yield_type = mpm::boundsurfplasticity::FailureState::Yield;
