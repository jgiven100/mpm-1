...
TEST_CASE("Bound Surface Plasticity (non-proportional) is checked in 3D", "[material][BoundSurfPlasticity][3D]") {
  // Tolerance
  ...
  jmaterial["d0"] = 2.5;
  jmaterial["fp"] = 0.5;
  jmaterial["proportional"] = false;
  jmaterial["cz"] = 20;
  jmaterial["zm"] = 5.5;

  // Check circular loading path
  SECTION("BoundSurfPlasticity circular loading path") {
    unsigned id = 0;
    auto material =
    ...
    std::ofstream myfile;
    myfile.open("non-prop-circular.txt");
  ...
  // // Check elliptical loading path
  // SECTION("BoundSurfPlasticity elliptical loading path") {
  //   unsigned id = 0;
  //   auto material =
  //       Factory<mpm::Material<Dim>, unsigned, const Json&>::instance()->create(
  //           "BoundSurfPlasticity3D", std::move(id), jmaterial);
  ...
  //   mpm::dense_map state_vars = material->initialise_state_variables();
  //   std::ofstream myfile;
  //   myfile.open("non-prop-elliptical.txt");
  ...
  // // Check figure-8 loading path
  // SECTION("BoundSurfPlasticity figure-8 loading path") {
  //   unsigned id = 0;
  //   auto material =
  //       Factory<mpm::Material<Dim>, unsigned, const Json&>::instance()->create(
  //           "BoundSurfPlasticity3D", std::move(id), jmaterial);
  ...
  //   mpm::dense_map state_vars = material->initialise_state_variables();
  //   std::ofstream myfile;
  //   myfile.open("non-prop-figure8.txt");
